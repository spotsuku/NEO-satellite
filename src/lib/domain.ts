...
export const TRIGGER_CRITERIA: Record<string, string> = {
  ...
  T3: "地域の旗を振る会長と事務局リーダー（若手人材）の両方が決まり、それぞれ本人が就任を引き受けている。両方そろった時点で成立。",
  T4: "準備室5ロール（現地紹介者・オーナー企業候補・学生リーダー候補・大学高校関係者・自治体関係者）がすべて「合意」になっている。充足すると成立提案バナーが出るが、確定は人の記録操作。",
  T5: "現地説明会を開催し、参加者から個別相談・追加の紹介・参画意向など次につながる反応が出ている。",
  T6: "オーナー候補3社（1000万円×1口）それぞれの経営トップとの会談で、出資検討の意思（金額感・社内検討の約束など）が表明される。3社そろった時点で成立。",
  T7: "1社目のオーナー企業との加盟契約の調印が完了している。",
  T8: "ステータス「確定」の加盟金合計が拠点目標（既定3,000万円＝1,000万円×3口）に到達している。1社確定するごとにチェックが埋まる。T1成立から90日以内が期限。到達すると成立提案バナーが出る。",
  T9: "キックオフ（開校）イベントを実施し、拠点の本番運営が始まっている。",
};
...
export const TRIGGER_CHECKLIST: Record<string, string[]> = {
  ...
  T5: ["現地説明会を開催する", "参加者から個別相談・追加の紹介・参画意向が出る"],
  T6: [
    "1社目：経営トップと会談し出資意思（1000万×1口）を確認",
    "2社目：経営トップと会談し出資意思を確認",
    "3社目：経営トップと会談し出資意思を確認",
  ],
  T7: ["1社目のオーナー企業と加盟条件に合意する", "調印式を実施する"],
  T8: [
    "1社目：加盟金1000万円（1口）が確定",
    "2社目：加盟金1000万円が確定",
    "3社目：加盟金1000万円が確定",
  ],
  T9: ["キックオフ（開校）イベントを実施する", "拠点の本番運営が始まる"],
};
...
// 90日時計: clockStart（T1成立日）から deadlineDays 後が加盟金トリガーの期限
