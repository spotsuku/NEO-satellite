...
サテライト拠点（熊本・長崎・佐賀・大分、以後追加あり）の立ち上げ進捗を、**トリガーイベント（T1〜T9）の成立**を背骨に可視化する社内ダッシュボード。
...
- **拠点ボード**: 9ステップ凡例帯 / 全社・拠点別 KPI / 拠点比較カード（県シルエット水位・T1〜T9チェーン・加盟金3色バー・燃料4指標・準備室5ドット・90日残・NEXT・停滞バッジ）/ 拠点詳細（3層ゲージ・準備室ロール・NEXT TRIGGER・トリガーログ・成立記録）
- **ステークホルダー**: 全拠点横断テーブル（拠点/カテゴリ/検索フィルタ、ステータス・金額・次回アクションのインライン編集、停滞アラート、CSVエクスポート）
- **関係図マップ**: ドラッグ移動・端子ドラッグで接続（4色）・線クリック削除・拠点切替・配置リセット。Supabase モードでは座標（0-1比率）とエッジを `map_nodes`/`map_edges` に永続化し、Realtime で共同編集反映（last-write-wins）。未配置のステークホルダーはプールに表示されワンクリックで配置
- **設定** (`/settings`): トリガー文言・確度係数・燃料目標・拠点別目標額の編集、拠点追加（県名から `dataofjapan/land` GeoJSON でシルエットSVGを自動生成）
...
- **Phase 1（済）**: スキーマ+seed / ボード / 拠点詳細（成立記録）/ ステークホルダーテーブル / 成立演出 / 記録者名UI / Excel seed
- **Phase 2（済）**: マップの Supabase 永続化・Realtime 共同編集（未配置プール含む）/ ステークホルダー新規追加 / 準備室ロールの状態更新UI / 燃料記録 / 他画面同時演出（activities 起点のリッチ表示・自画面二重発火防止）
- **Phase 3（済）**: 設定画面（トリガー文言・確度係数・燃料目標・拠点別目標）/ 拠点追加UI（県名からシルエット自動生成）/ `scripts/gen-silhouette.ts`
- **今後**: 燃料推移ミニチャート / n8n フローの実接続（下記手順）/ マップのフリーノード
